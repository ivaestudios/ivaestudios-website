...
(function () {
  ...
  var uid = 0;
  function svgOpen(w) {
    return '<svg viewBox="0 0 200 430" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"' + (w ? ' class="' + w + '"' : '') + '>';
  }
  function defs(c) {
    uid++;
    var g = 'osxg' + uid;
  ...
  function figHombre(c, skin) {
    ...
    return svgOpen() + d.html + SHADOW + RING +
      '<ellipse cx="100" cy="44" rx="13.5" ry="15.5" fill="' + skin + '"/>' +
      '<path d="M85,40 Q85,24 100,24 Q115,24 115,40 L113,47 Q100,36 87,47 Z" fill="' + HAIR + '"/>' +
      '<rect x="94" y="57" width="12" height="14" rx="4" fill="' + skin + '"/>' +
      '<path d="M78,80 C73,116 74,152 82,192 L118,192 C126,152 127,116 122,80 Q112,72 100,74 Q88,72 78,80 Z" fill="url(#' + d.id + ')"/>' +
      '<path d="M92,75 L100,91 L108,75" stroke="' + c2 + '" stroke-width="2.2" fill="none"/>' +
      '<line x1="100" y1="91" x2="100" y2="188" stroke="' + c2 + '" stroke-width="1.6" opacity=".5"/>' +
      '<path d="M78,82 C64,104 58,128 60,148 L74,152 C72,130 76,108 84,90 Z" fill="url(#' + d.id + ')"/>' +
      '<path d="M122,82 C136,104 142,128 140,148 L126,152 C128,130 124,108 116,90 Z" fill="url(#' + d.id + ')"/>' +
      '<path d="M68,150 L71,178" stroke="' + skin + '" stroke-width="7" fill="none" stroke-linecap="round"/>' +
      '<path d="M132,150 L129,178" stroke="' + skin + '" stroke-width="7" fill="none" stroke-linecap="round"/>' +
      '<path d="M81,190 L99,190 L98,295 L97,402 L82,402 L84,295 Z" fill="' + pant + '"/>' +
      '<path d="M101,190 L119,190 L116,295 L118,402 L103,402 L102,295 Z" fill="' + pant + '"/>' +
      '<path d="M90,198 L89,394" stroke="' + pant2 + '" stroke-width="1.6" opacity=".7"/>' +
      '<path d="M110,198 L111,394" stroke="' + pant2 + '" stroke-width="1.6" opacity=".7"/>' +
  ...
  function figNina(c, skin) {
    var d = defs(c), c2 = shade(c, -0.24);
    return svgOpen() + d.html + SHADOW + RING +
      '<ellipse cx="100" cy="62" rx="20" ry="22" fill="' + HAIR + '"/>' +
      '<circle cx="78" cy="48" r="8" fill="' + HAIR + '"/>' +
      '<circle cx="122" cy="48" r="8" fill="' + HAIR + '"/>' +
      '<ellipse cx="100" cy="66" rx="15" ry="17" fill="' + skin + '"/>' +
      ...
      '<path d="M90,274 C89,318 88,362 90,398" stroke="' + skin + '" stroke-width="7" fill="none" stroke-linecap="round"/>' +
      '<path d="M110,274 C111,318 112,362 110,398" stroke="' + skin + '" stroke-width="7" fill="none" stroke-linecap="round"/>' +
      '<ellipse cx="89" cy="404" rx="6.5" ry="3.4" fill="' + skin + '"/>' +
      '<ellipse cx="111" cy="404" rx="6.5" ry="3.4" fill="' + skin + '"/>' +
      '</svg>';
  }
  function figNino(c, skin) {
    var d = defs(c), c2 = shade(c, -0.26), pant = '#DCD2BA';
    return svgOpen() + d.html + SHADOW + RING +
      '<ellipse cx="100" cy="64" rx="16" ry="18" fill="' + skin + '"/>' +
      '<path d="M83,58 Q83,40 100,40 Q117,40 117,58 L115,64 Q100,52 85,64 Z" fill="' + HAIR + '"/>' +
      ...
      '<path d="M85,176 L115,176 L113,240 L103,240 L101,198 L99,198 L97,240 L87,240 Z" fill="' + pant + '"/>' +
      '<path d="M91,244 C90,296 89,352 91,398" stroke="' + skin + '" stroke-width="7" fill="none" stroke-linecap="round"/>' +
      '<path d="M109,244 C110,296 111,352 109,398" stroke="' + skin + '" stroke-width="7" fill="none" stroke-linecap="round"/>' +
      '<ellipse cx="90" cy="404" rx="6.5" ry="3.4" fill="' + skin + '"/>' +
      '<ellipse cx="110" cy="404" rx="6.5" ry="3.4" fill="' + skin + '"/>' +
      '</svg>';
  }
  ...
  var T = ES ? {
    ...
    verdict: 'Veredicto de tu estilista', cta: 'Enviar mi combinación a Vianey',
    empty: 'Agrega a las personas de tu sesión para armar su look.',
    waIntro: 'Hola Vianey, armé mi outfit en la guía de vestuario: ',
    waAsk: ' ¿Qué opinas?',
    askPlaceholder: 'Escribe tu duda, por ejemplo: ¿qué me pongo si hace viento?',
    ...
    verdict: 'Your stylist verdict', cta: 'Send my combo to Vianey',
    ...
    askPlaceholder: 'Type your question, e.g.: what should I wear if it is windy?',
  ...
  function analyze() {
    ...
    if (has('black')) add('bad',
      'El negro absorbe la luz de la playa y se ve plano en foto. Cámbialo por azul marino: da la misma elegancia con vida.',
      'Black absorbs beach light and looks flat on camera. Swap it for navy: same elegance, more life.');
    if (has('coral')) add('bad',
      'Los colores brillantes rebotan tinte en la piel. Prueba terracota: cálido, rico y sin reflejos.',
    ...
    if (p.length >= 2) {
      ...
      if (allSame && (ids[0] === 'cream' || ids[0] === 'white')) add('ok',
        ...
        'Coordinen, no se uniformen: dejen un color ancla y varíen los demás con tonos vecinos.',
    ...
    if (sc === 'selva' && has('sage')) add('meh',
      'El salvia se camuflea con el verde de la selva. Contrasta con crema o rosa palo para destacar.',
  ...
  function renderProbador() {
    ...
    var html = '<div class="osx-wrap"><div class="osx-grid"><div>' +
      '<div class="osx-stage">' +
    ...
    for (var i = 0; i < SCENES.length; i++) {
      var s = SCENES[i];
      html += '<button type="button" class="osx-scene' + (s.id === state.scene ? ' sel' : '') + '" data-scene="' + s.id + '">' +
    ...
    for (var j = 0; j < state.people.length; j++) {
      ...
      html += '<span class="osx-chip' + (j === state.sel ? ' sel' : '') + '" data-i="' + j + '" role="button" tabindex="0">' +
        '<span class="dot" style="background:' + colorById(per.color).hex + '"></span>' + (ES ? f2.es : f2.en) +
        '<span class="x" data-del="' + j + '" aria-label="' + (ES ? 'Quitar' : 'Remove') + '">&times;</span></span>';
    }
    if (state.people.length < 5)
      html += '<button type="button" class="osx-add" id="osxAdd">+ ' + T.add + '</button>';
    html += '</div><div class="osx-addmenu" id="osxAddMenu">';
    ...
    if (selPer) {
      ...
      html += '<div class="osx-panel"><p class="osx-panel-t">' + T.outfit + ' <b>' + fName + '</b></p><div class="osx-swatches">';
      for (var m = 0; m < COLORS.length; m++) {
        var c = COLORS[m];
        html += '<button type="button" class="osx-sw' + (c.id === selPer.color ? ' sel' : '') + '" data-color="' + c.id + '" style="background:' + c.hex + '" aria-label="' + (ES ? c.es : c.en) + '" title="' + (ES ? c.es : c.en) + '"></button>';
      }
      html += '</div><p class="osx-panel-t" style="margin-top:16px">' + T.skin + '</p><div class="osx-swatches osx-skins">';
      for (var n = 0; n < SKINS.length; n++)
        html += '<button type="button" class="osx-sw' + (n === selPer.skin ? ' sel' : '') + '" data-skin="' + n + '" style="background:' + SKINS[n] + '" aria-label="' + T.skin + ' ' + (n + 1) + '"></button>';
      html += '</div></div>';
    }
    ...
    html += '<div class="osx-verdict"><div class="osx-verdict-head">' +
  ...
  function wireProbador() {
    probadorMount.querySelectorAll('.osx-scene').forEach(function (b) {
      b.addEventListener('click', function () { state.scene = b.getAttribute('data-scene'); renderProbador(); });
    });
    ...
    var add = document.getElementById('osxAdd'), menu = document.getElementById('osxAddMenu');
    if (add) add.addEventListener('click', function () { menu.classList.toggle('open'); });
    probadorMount.querySelectorAll('[data-add]').forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        state.sel = state.people.length - 1; renderProbador();
      });
    });
    probadorMount.querySelectorAll('[data-color]').forEach(function (b) {
      b.addEventListener('click', function () { state.people[state.sel].color = b.getAttribute('data-color'); renderProbador(); });
    });
    probadorMount.querySelectorAll('[data-skin]').forEach(function (b) {
      b.addEventListener('click', function () { state.people[state.sel].skin = parseInt(b.getAttribute('data-skin'), 10); renderProbador(); });
    });
  }
  ...
  var QA = ES ? [
    ...
    { c: 'hombres', q: '¿Qué se pone él?', k: ['hombre', 'el', 'camisa', 'pantalon', 'caballero', 'senor'], a: 'El uniforme ganador: camisa de lino (crema, arena, salvia o marino) con pantalón de lino claro, arremangada y con uno o dos botones abiertos. Descalzo en arena o con sandalias de piel. Sin relojes deportivos ni lentes en la cabeza.' },
    ...
    { c: 'vestido', q: '¿Vestido largo o corto?', k: ['vestido', 'largo', 'corto', 'maxi'], a: 'El maxi vestido fluido es el rey de la playa: el viento lo convierte en movimiento y drama editorial. El corto funciona lindo en sesiones casuales o si te sientes más tú. Si dudas, lleva ambos: hacemos cambio de outfit.' },
    ...
    { c: 'clima', q: '¿Y si hace mucho calor?', k: ['calor', 'sudor', 'fresco'], a: 'Lino y algodón respiran; el poliéster no. Trae tu outfit en gancho y cámbiate al llegar, agua fría y toallitas de papel secante. Las sesiones al atardecer también son más frescas que al mediodía.' },
    { c: 'clima', q: '¿Qué pasa si llueve el día de la sesión?', k: ['lluvia', 'llueve', 'clima', 'tormenta'], a: 'Tenemos plan B siempre: movemos la hora al claro del día o reagendamos sin costo. Y un secreto: el cielo después de la lluvia da la luz más dramática y bonita de todas.' },
    ...
    { c: 'detalles', q: '¿Tacones o descalza?', k: ['tacones', 'zapatos', 'descalza', 'sandalias', 'calzado'], a: 'Descalza en la arena, siempre: es natural, cómodo y estiliza igual que un tacón en foto de cuerpo completo. Para hotel o ruinas, sandalias planas de piel o un tacón bajo bloque si el suelo lo permite.' },
    { c: 'practico', q: '¿Cuántos cambios de outfit llevo?', k: ['cambio', 'outfit', 'cuantos', 'ropa'], a: 'Dos es el número ideal en una sesión estándar: uno fluido y romántico para golden hour, otro más casual o con color. Llévalos planchados y en gancho, y nosotros coordinamos el momento del cambio.' },
    ...
    { c: 'practico', q: '¿Puedo pedir asesoría personal de vestuario?', k: ['asesoria', 'ayuda', 'personal', 'vianey', 'duda'], a: 'Claro que sí: mándanos foto de tus opciones por WhatsApp y te decimos qué funciona mejor para tu locación y horario. Es parte de prepararte bien, sin costo extra.' }
  ] : [
    ...
    { c: 'men', q: 'What should he wear?', k: ['men', 'him', 'shirt', 'pants', 'guy'], a: 'The winning uniform: a linen shirt (cream, sand, sage or navy) with light linen pants, sleeves rolled, a button or two open. Barefoot on sand or leather sandals. No sport watches, no sunglasses on the head.' },
    ...
    { c: 'dresses', q: 'Long dress or short?', k: ['dress', 'long', 'short', 'maxi'], a: 'The flowing maxi is the queen of the beach: wind turns it into movement and editorial drama. Short works beautifully for casual sessions or if it feels more you. In doubt, bring both: we do an outfit change.' },
    ...
    { c: 'weather', q: 'What if it is very hot?', k: ['hot', 'heat', 'sweat'], a: 'Linen and cotton breathe; polyester does not. Bring your outfit on a hanger and change on arrival, plus cold water and blotting papers. Sunset sessions also run much cooler than midday.' },
    { c: 'weather', q: 'What happens if it rains on session day?', k: ['rain', 'rains', 'weather', 'storm'], a: 'There is always a plan B: we shift the hour to the clear window or reschedule at no cost. And a secret: post-rain skies give the most dramatic, beautiful light of all.' },
    ...
    { c: 'practical', q: 'How many outfit changes should I bring?', k: ['changes', 'outfits', 'how many'], a: 'Two is the sweet spot for a standard session: one flowing and romantic for golden hour, one more casual or with color. Bring them pressed on hangers and we coordinate the change moment.' },
    ...
    { c: 'practical', q: 'Can I get personal outfit advice?', k: ['advice', 'help', 'personal', 'vianey', 'question'], a: 'Of course: send us photos of your options on WhatsApp and we will tell you what works best for your location and hour. It is part of preparing you well, at no extra cost.' }
  ];
    ...
    for (var i = 0; i < QA.length; i++) {
      ...
      for (var t = 0; t < toks.length; t++) {
        ...
        for (var kk = 0; kk < qa.k.length; kk++) {
          var key = norm(qa.k[kk]);
          if (key.indexOf(tok) !== -1 || tok.indexOf(key) !== -1) { score += 2; break; }
        }
  ...
  function renderPreguntas() {
    ...
    var html = '<div class="osx-ask"><div class="osx-askbox">' +
      '<input type="text" id="osxAskInput" placeholder="' + T.askPlaceholder + '" autocomplete="off" aria-label="' + T.askPlaceholder + '"/>' +
      '<span class="lens"><svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path stroke-linecap="round" d="M21 21l-4.3-4.3"/></svg></span></div>' +
