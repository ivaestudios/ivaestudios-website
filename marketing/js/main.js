...
import * as shell from './shell/shell.js?v=202609121529';
import { el } from './api.js?v=202609121529';
import { icon } from './shell/icons.js?v=202609121529';
...
lazyView({ id: 'inicio', label: T('Inicio', 'Home'), icon: 'home', loader: () => import('./views/dashboard.js?v=202609121529') });
...
lazyView({ id: 'meses', label: T('Calendario', 'Calendar'), icon: 'calendar', loader: () => import('./views/meses.js?v=202609121529') });
lazyView({ id: 'calendario', label: T('Cuadrícula', 'Grid'), icon: 'grip', loader: () => import('./calendar/index.js?v=202609121529') });
lazyView({ id: 'tablero', label: T('Tablero', 'Board'), icon: 'board', loader: () => import('./views/kanban.js?v=202609121529') });
lazyView({ id: 'tabla', label: T('Tabla', 'Table'), icon: 'table', loader: () => import('./views/table.js?v=202609121529') });
lazyView({ id: 'timeline', label: 'Timeline', icon: 'gantt', loader: () => import('./views/timeline.js?v=202609121529') });
lazyView({ id: 'carga', label: T('Carga', 'Workload'), icon: 'gauge', loader: () => import('./views/workload.js?v=202609121529') });
lazyView({ id: 'mi-trabajo', label: T('Mi trabajo', 'My work'), icon: 'briefcase', loader: () => import('./views/mywork.js?v=202609121529') });
lazyView({ id: 'automatizaciones', label: T('Automatizaciones', 'Automations'), icon: 'zap', loader: () => import('./views/automations.js?v=202609121529') });
// Métricas: panel de Instagram por periodo (semana/mes/3-6 meses/año/custom).
lazyView({ id: 'metricas', label: T('Métricas', 'Metrics'), icon: 'gauge', loader: () => import('./views/metricas.js?v=202609121529') });
// Conexiones: semáforo de redes por marca + botones de conectar (solo staff).
lazyView({ id: 'conexiones', label: T('Conexiones', 'Connections'), icon: 'link', loader: () => import('./views/conexiones.js?v=202609121529') });
// Estudios: el padrón de IVAE Gallery (fotógrafos registrados, plan, pago, uso). Solo staff.
lazyView({ id: 'estudios', label: T('Estudios', 'Studios'), icon: 'users', loader: () => import('./views/estudios.js?v=202609121529') });
// Entregables: contenido final (reels + carruseles) que el cliente ve y descarga.
lazyView({ id: 'entregables', label: T('Entregables', 'Deliverables'), icon: 'archive', loader: () => import('./views/entregables.js?v=202609121529') });
// Feed: el perfil de Instagram como va a quedar (publicado + programado).
lazyView({ id: 'feed', label: T('Feed', 'Feed'), icon: 'grip', loader: () => import('./views/feed.js?v=202609121529') });
// Marca: el manual de identidad (PDF) que el cliente consulta y el staff sube.
lazyView({ id: 'marca', label: T('Marca', 'Brand'), icon: 'book', loader: () => import('./views/marca.js?v=202609121529') });
// Carrusel: cortador de tiras de carrusel en slides descargables (solo staff).
lazyView({ id: 'carrusel', label: T('Carrusel', 'Carousel'), icon: 'scissors', loader: () => import('./views/carrusel.js?v=202609121529') });
// Descargar: baja videos de IG/TikTok/Pinterest sin marca de agua (solo staff).
lazyView({ id: 'descargar', label: T('Descargar', 'Download'), icon: 'download', loader: () => import('./views/descargar.js?v=202609121529') });
// Video IA: genera clips con IA (fal.ai) y los anota por marca (solo staff).
lazyView({ id: 'video-ia', label: T('Video IA', 'AI video'), icon: 'spark', loader: () => import('./views/video-ia.js?v=202609121529') });

// Detalle de post (#/post/:id): el deep-link que usan busqueda y avisos.
lazyView({ id: 'post', label: T('Contenido', 'Content'), icon: 'edit', loader: () => import('./editor/editor.js?v=202609121529') });
...
if ('serviceWorker' in navigator) {
  const registerSw = () => {
    navigator.serviceWorker
      .register('/marketing/sw.js?v=202609121529', { scope: '/marketing/' })
