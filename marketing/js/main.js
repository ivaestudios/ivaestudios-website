...
lazyView({ id: 'carga', label: 'Carga', icon: 'gauge', loader: () => import('./views/workload.js') });
lazyView({ id: 'mi-trabajo', label: 'Mi trabajo', icon: 'briefcase', loader: () => import('./views/mywork.js') });
lazyView({ id: 'automatizaciones', label: 'Automatizaciones', icon: 'zap', loader: () => import('./views/automations.js') });

// Detalle de post (#/post/:id): el deep-link que usan busqueda y avisos.
lazyView({ id: 'post', label: 'Contenido', icon: 'edit', loader: () => import('./editor/editor.js') });
