...
import { api, el, clear, avatar, timeAgo, initials, copyText, esCreador } from '../api.js?v=202609251414';
import * as store from './store.js?v=202609251414';
import { openSheet, pickFrom } from './sheet.js?v=202609251414';
import { toast } from './toast.js?v=202609251414';
import { icon } from './icons.js?v=202609251414';
import { openClientSwitcher } from './clientswitcher.js?v=202609251414';
import { T, isEN, setLang } from './i18n.js?v=202609251414';
// Apple 1.2: lista de personas bloqueadas desde el menú de cuenta.
import { abrirBloqueados } from './moderacion.js?v=202609251414';
import { getTheme, setTheme } from './theme.js?v=202609251414';
import * as version from './version.js?v=202609251414';
import * as tienda from './tienda.js?v=202609251414';
import { abrirAjustesAvisos } from './avisos-ajustes.js?v=202609251414';
...
const DESKTOP_TABS = [
  ...
  // Bandeja: comentarios + mensajes + CRM por marca (solo staff; el filtro de
  // cliente de abajo es lista blanca y no la incluye).
  { id: 'bandeja', label: T('Bandeja', 'Inbox') },
  { id: 'meses', label: T('Calendario', 'Calendar') },
  { id: 'calendario', label: T('Cuadrícula', 'Grid') },
  // ⚠️ Una vista solo aparece si está EN ESTA LISTA: ponerla en DESKTOP_GRUPOS
  // no basta (el grupo filtra por los ids que existen aquí). A Feed le pasó:
  // estaba en el grupo Contenido y no se pintaba en ningún menú.
  { id: 'feed', label: 'Feed' },
  { id: 'entregables', label: T('Entregables', 'Deliverables') },
  { id: 'marca', label: T('Marca', 'Brand') },
  { id: 'carrusel', label: T('Carrusel', 'Carousel') },
  { id: 'descargar', label: T('Descargar', 'Download') },
  { id: 'video-ia', label: T('Video IA', 'AI video') },
  ...
  // Pauta: anuncios pagados de la CASA. Solo admin (el filtro de rol la quita
  // para equipo y cliente); es dinero de IVAE, no de una marca.
  { id: 'pauta', label: T('Pauta', 'Ads') },
...
const DESKTOP_GRUPOS = [
  ...
  { id: 'g-bandeja', label: T('Bandeja', 'Inbox'), items: ['bandeja'] },
  ...
  { id: 'g-resultados', label: T('Resultados', 'Results'), items: ['metricas', 'conexiones', 'pauta'] },
];
...
export function createTopbar({ root, router, selectClient, openSearch, openNotifications }) {
  ...
  // Conexiones para clientes: lista blanca por marca (pedido 2026-08-27, solo
  // Regeneris). MISMA lista en shell.js y conexiones.js — cámbialas juntas.
  const CLIENT_CONEXIONES_IDS = ['demo-regeneris'];
  const clientSeesConexiones = isClient && CLIENT_CONEXIONES_IDS.includes((store.getState().me || {}).client_id);
  // Pauta es de la DUENA: el equipo tampoco la ve (el backend ya responde 403,
  // esto solo evita ofrecer una puerta que siempre se cierra).
  const esAdmin = ((store.getState().me || {}).role === 'admin');
  const visibleTabs = isClient
    ...
    : DESKTOP_TABS.filter((t) => t.id !== 'pauta' || esAdmin);
  const irA = (id) => {
    ...
    openSheet({
      ...
      build(body, close) {
        body.append(
          ...
          themeRow(),
          versionRow(),
          // SOLO dentro de la app de la App Store: el renglon de arriba actualiza
          // el CODIGO WEB, pero el envoltorio nativo solo cambia desde la
          // tienda. Sin esto el cliente no tiene por donde enterarse. En el
          // navegador no sale: ahi no hay nada que actualizar (ver tienda.js).
          ...(tienda.enLaApp() ? [
            accountRow('link', T('Actualizar la app', 'Update the app'), () => {
              close();
              window.open(tienda.APP_STORE_URL, '_blank', 'noopener');
            }),
          ] : []),
          ...
          accountRow('link', T('Términos de Uso', 'Terms of Use'), () => {
            close();
            window.open('/marketing/terminos', '_blank', 'noopener');
          }),
  ...
  // La tarjeta "Empieza aquí" del calendario pide crear el acceso de SU marca.
  if (!window.__mktCrearAcceso) {
    window.__mktCrearAcceso = true;
    window.addEventListener('mkt:crear-acceso', (e) => {
      const id = e && e.detail && e.detail.clientId;
      const brand = (store.getState().clients || []).find((c) => c.id === id);
      if (brand) openCreateClientLogin(brand); else openClientAccessSheet();
    });
  }
