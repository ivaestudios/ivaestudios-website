...
import { renderGen, resetGen } from './carrusel-gen.js?v=202608010231';
...
async function cutVideoWebCodecs() {
  ...
  const { Muxer, ArrayBufferTarget } = await import('../vendor/mp4-muxer.mjs?v=202608010231');
  const cols2 = vcols, rows2 = vrows, n = cols2 * rows2;
  const sw = Math.floor(v.videoWidth / cols2), sh = Math.floor(v.videoHeight / rows2);
...
function ensureCss() {
  ...
  link.rel = 'stylesheet';
  link.href = '/marketing/css/carrusel.css?v=202608010231';
