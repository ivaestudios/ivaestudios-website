...
} from '../api.js?v=202609161438';
import { icon, iconMarca } from '../shell/icons.js?v=202609161438';
import { T } from '../shell/i18n.js?v=202609161438';
import { ACTION_LABELS, detalleEvento } from '../lib/actividad-fmt.js?v=202609161438';
import { confirmar } from '../shell/sheet.js?v=202609161438';
import { openEditClient, openNewClient } from '../shell/clientswitcher.js?v=202609161438';
// Tarjeta compartida "Error + Reintentar" (la misma de Inicio / Mi trabajo).
import { errorCard } from '../ui/states.js?v=202609161438';
import { buildInsertUpdates } from '../kanban/move-sheet.js?v=202609161438';
// El panel del guion vive fuera: lo comparten esta vista y la Cuadricula.
import { abrirGuion, cerrarGuion, vaciarPortapapeles as vaciarPortapapelesEn } from '../lib/guion-drawer.js?v=202609161438';
...
} from '../lib/video-upload.js?v=202609161438';
...
const vaciarPortapapeles = (mensaje) => vaciarPortapapelesEn(ctx, mensaje);

// ── Copiar caption + hashtags SIN abrir la pieza ─────────────────────────────
// Pedido de Vianey (2026-08-29): lo que ella hace todo el dia es pegar el
// caption en Instagram, y para eso tenia que abrir el panel de guion pieza por
// pieza. El boton vive AFUERA, en la propia fila (escritorio) y tarjeta (movil),
// y copia exactamente lo mismo que "Copiar caption + hashtags" del panel.
//
// Lee del STORE, no del snapshot con el que se pinto la fila: si el caption se
// acaba de editar en el panel o desde otra pestana, la fila puede traer el
// texto viejo y se copiaria la version anterior.
async function copiarCaptionYHashtags(post) {
  const { posts } = ctx.store.getState();
  const p = (posts || []).find((x) => String(x.id) === String(post.id)) || post;
  const caption = String(p.caption || '').trim();
  const tags = String(p.hashtags || '').trim();
  const texto = [caption, tags].filter(Boolean).join('\n\n');
  // Ley del portapapeles: copiar "nada" lo VACIA, nunca lo deja intacto
  // (ver vaciarPortapapeles: un pegado con el caption de otra pieza ya se
  // publico una vez en el Instagram de un cliente).
  if (!texto) {
    await vaciarPortapapeles(T('Esta pieza no tiene caption ni hashtags.', 'This piece has no caption or hashtags.'));
    return;
  }
  const ok = await copyText(texto);
  if (!ok) { ctx.toast(T('No se pudo copiar.', 'Could not copy.'), { type: 'error' }); return; }
  // Decir QUE se copio: si la pieza no trae hashtags, que se entere aqui y no
  // al pegar en Instagram.
  ctx.toast(
    tags
      ? T('Caption + hashtags copiados.', 'Caption + hashtags copied.')
      : T('Caption copiado. Esta pieza no tiene hashtags.', 'Caption copied. This piece has no hashtags.'),
    { type: tags ? 'success' : 'info', ms: tags ? 2500 : 6000 },
  );
}

// El boton, compartido por la fila del escritorio y la tarjeta del movil.
// stopPropagation es OBLIGATORIO: sin el, el clic sigue hasta el boton-celda
// (o la tarjeta) de atras y abre el panel encima del toast.
function botonCopiarCaption(post, clase, etiqueta) {
  const b = el('button', {
    class: clase, type: 'button',
    title: T('Copiar caption + hashtags', 'Copy caption + hashtags'),
    'aria-label': `${T('Copiar caption y hashtags de', 'Copy caption and hashtags for')} ${post.title || T('esta pieza', 'this piece')}`,
  }, [icon('copy', etiqueta ? 13 : 15), etiqueta ? el('span', { text: etiqueta }) : null].filter(Boolean));
  b.addEventListener('click', (e) => {
    e.stopPropagation(); e.preventDefault();
    copiarCaptionYHashtags(post);
  });
  return b;
}
...
const PLAT_ICONO = {
  ...
  tiktok: 'tiktok', 'tik tok': 'tiktok', youtube: 'youtube', 'you tube': 'youtube',
};
function platformNode(platform) {
  ...
  const nodos = redes.map((nombre) => {
    ...
    const logo = PLAT_ICONO[clave] ? iconMarca(PLAT_ICONO[clave], 17) : null;
    return logo || el('span', { class: 'meses-pill meses-pill--plain', text: nombre });
  });
...
function buildRow(post, noteLabels) {
  ...
  const avisoNode = (soyAdmin && !yaPaso && avisoTexto)
    ...
    : null;
  if (avisoNode) avisoNode.addEventListener('click', (e) => { e.stopPropagation(); abrirGuion(post, {
      ctx, trackMutation, alGuardar: descartarSiEsDeOtraMarca, avisarCaption: avisarCaptionSucio,
    }); });
  // Copiar caption + hashtags SIN abrir la pieza. Va en la columna Tarea y no
  // en la de Guiones porque Tarea es la única sticky: la tabla mide ~1900px en
  // un hueco de ~1140px, así que un botón en Guiones queda fuera de la parte
  // visible hasta que se desliza a mano (medido, no supuesto).
  // Mismo candado que el móvil: el cliente no copia captions.
  const copyBtn = isClientRole() ? null : botonCopiarCaption(post, 'meses-task__copy');
  tdTask.appendChild(el('span', { class: 'meses-task' }, [dragBtn, titleBtn, avisoNode, copyBtn, openBtn, deleteBtn].filter(Boolean)));
...
function buildMobileItem(post, noteLabels) {
  ...
  // Va aquí y no en la cabecera porque allá un botón más dejaba el título en
  // "Carrusel 1…" (medido en iPhone), y porque en el teléfono un ícono suelto
  // no dice qué copia.
  if (!isClientRole()) {
    chips.appendChild(botonCopiarCaption(post, 'meses-chip meses-chip--copy', T('Copiar caption + hashtags', 'Copy caption + hashtags')));
  }
...
function buildPdfContenidoBtn(key, rows) {
  ...
  return el('button', {
    ...
    onclick: async (e) => {
      ...
      btn.disabled = true;
      try {
        const mod = await import('../lib/pdf-contenido.js?v=202609161438');
        const { vozDeMarca } = await import('../lib/pdf-lienzo.js?v=202609161438');
...
function buildSection({ key, rows, noteLabels, collapsed = false, desktop, isTodos, single = false }) {
  ...
  if (rows.length) {
    ...
    bodyKids.push(el('div', { class: 'meses-empty empty-rich' }, [
      ...
      el('p', { class: 'empty-rich__s', text: T('Aun no hay contenidos en este mes. Agrega la primera línea abajo para arrancar.', 'No content in this month yet. Add the first row below to get started.') }),
    ]));
  }
...
export default {
  ...
  mount(host, c) {
    ...
    unsubs.push(
      ...
      ctx.store.on('client:changed', () => { composer = null; composerInput = null; activeMonth = null; cerrarGuion({ force: true }); }),
    );
